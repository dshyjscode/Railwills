...
function SPlanReceiptJson_to_TD(){
	...
	json.k=k
	//发送给列调回执
	addCookie('济南车务终端回执',json.recJson)
		...
		if(val=="01"){
			...
			if(document.getElementById("SendOrderNum").innerHTML==""){
                ...
                if(model==1){  //如果是自律模式，不允许更改股道
					...
					document.getElementById("jinan_stationtrack_TX").value=track;
				}
...
/*function getCookie_receipt(){
	...
	//存储回执信息
	if(timer==2){
		$.each(cookie_jn,function(key,val){

...
function SPlanReceiptJson_to_TD(){
	...
	json.k=k
	//发送给列调回执
	addCookie('天津车务终端回执',json.recJson)
	...
	if(Object.keys(show.carWhere).length==1){
        ...
        show.car.forEach(function(val,n){
            if(show.td_PlanShow[n].substr(36,2)=="02"){
				...
				rsType[val]="通过"
                rsTime[val]=show.td_PlanShow[n].substr(32,4)
				t++
            }
            else if(show.td_PlanShow[n].substr(36,2)=="01"){
                rsType[val]="始发"
		...
		if(show.car.length==4 && n==2){
			
			if(d==2 && t==2){
				$.each(show.carWhere,function(key){
					if(rsType[key]=="到开"){
						show.carTrack[key]="3"  
					}
					else{
		...
		if(val=="01"){
			if(document.getElementById("RecOrderNum").innerHTML==""){
                ...
                if(model==1){  //如果是自律模式，不允许更改股道
					...
					document.getElementById("tianjin_stationtrack_RX").value=track;
				}
			...
			if(document.getElementById("SendOrderNum").innerHTML==""){
                ...
                if(model==1){  //如果是自律模式，不允许更改股道
					...
					document.getElementById("tianjin_stationtrack_TX").value=track;
				}
				time = show.planTime[n].substr(0,2)+"时"+show.planTime[n].substr(2,2)+"分"
...
/*function getCookie_receipt(){
	...
	//存储回执信息
	if(timer==2){
		$.each(cookie_tj,function(key,val){
...
function addCookie_ceshi(){
	var json={0:"AB810201100020201015195643001111054601AC",1:"AB810101110120201015195643001110060603AC",2:"AB810201110220201015195643001112060603AC",3:"AB810101120320201015195643001111062504AC",4:"AB810201100420201015195643001311064201AC",5:"AB810101110520201015195643001310071004AC"}
	$.cookie('列调', json, { path: '/' });
}

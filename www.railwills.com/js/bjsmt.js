...
function get_moshizhuanhuan()
  {
   ...
   // alert("模式转换成功");
	...
	clearTimeout(canceljinlutimer);
   }
   Mode_Switching()//模式转换后的调车和列车操作
  }
...
function click_jdjhqc(){
	alert("计划签收成功")
}

function click_ddmlhz(){
	alert("命令回执成功");
	setCookie("DDMLHZ","1");
	var b=document.getElementById("ddml").value;
	setCookie("a",b);
}
...
function xd_Switch(){
	if(xd_model==1){
		xd_model=0
		document.getElementById("xd_Switch").innerHTML="调车"
	}
	else{
		xd_model=1
		document.getElementById("xd_Switch").innerHTML="行车"
	}
    Mode_Switching()//模式转换后的调车和列车操作 
}
//李紫玉Add

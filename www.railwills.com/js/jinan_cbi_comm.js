...
//进路信息数据帧编码
//每一位信息  帧头、类型、功能码、发送方、接收方、车次号、到发时间、进路序号、到发类型、帧尾
...
function CBIReceiptJson_to_SMT(){
	...
	//json.k=k
	//发送给济南车务终端回执
	addCookie('济南联锁回执',json.recJson)
	//alert(JSON.stringify( $.cookie('济南联锁回执')))
    json.recJson={}
    //存储车务终端计划在等待发送变量里
...
function CBI_to_STMap(){
	...
	//json.l=l
	//发送给站场图
	addCookie('济南联锁',json.routeAllNum)
	//alert(JSON.stringify( $.cookie('济南联锁')))
	//清空存储列调计划的变量

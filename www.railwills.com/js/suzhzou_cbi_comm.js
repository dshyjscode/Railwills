...
function CBIReceiptJson_to_SMT(){
	...
	//json.k=k
	//发送给苏州车务终端回执
	addCookie('苏州联锁回执',json.recJson)
	//alert(JSON.stringify( $.cookie('苏州联锁回执')))
    json.recJson={}
    //存储车务终端计划在等待发送变量里
...
function CBI_to_STMap(){
	...
	//json.l=l
	//发送给站场图
	addCookie('苏州联锁',json.routeAllNum)
	//alert(JSON.stringify( $.cookie('苏州联锁')))
	//清空存储列调计划的变量

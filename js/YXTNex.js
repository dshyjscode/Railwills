...
//Y
function coordY(e){
    var scroll=document.documentElement.scrollTop||document.body.scrollLeft,
        scY=$('#Sec').scrollTop(),
...
function allBG(){
    for(var i=0;i<60*15+1;i+=2){
        WLine(i*svg.TimeX,svg.bgH,i*svg.TimeX,3000+svg.bgH-100,'','#ccc',0,2,'allZ','allZ')       //纵线
    }
    for(var j=0;j<30;j++){
        WLine(0,100*j+svg.bgH,15*60*svg.TimeX,100*j+svg.bgH,'','#ccc',0,2,'allH','allH')       //横线
    }
    //AllPoints()
}
...
function AllPoints() {
    ...
    svg.AllPointsXZ.forEach(function(item,index){
        ...
        svg.AllPointsXH.forEach(function(item2,index2) {
            if(svg.AllPointsXZ[index]==svg.AllPointsXH[index2]){
...
function goDraw(CarNum,Point,read) {
    ...
    if(svg.newPoint.y!=null){
        //储存车次号。且车次号不可重复
            svg.Car.indexOf(CarNum)==-1?svg.Car.push(CarNum):""
//储存之后从总数据处删除该车次
        if(read=='read'){
        ...
        var text=Math.round(svg.newPoint.x/svg.TimeX)%60

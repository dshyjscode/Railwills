...
//联锁回执数据帧编码
//每一位信息  帧头、类型、功能码、发送方、接收方、命令序号、车次号、帧尾
function CBIReceiptFrame(head,type,fun_HZ,send,rec,csn,Car_Id,tail){
	
	var frame=head+type+fun_HZ+send+rec+csn+Car_Id+tail;
	return frame
}
//进路信息数据帧编码，联锁发送至站场图
...
function CBIReceiptJson_to_SMT(){
	...
	//json.k=k
	//发送给北京车务终端回执
	addCookie('北京联锁回执',json.recJson)
	//alert(JSON.stringify( $.cookie('北京联锁回执')))
    json.recJson={}
    //存储车务终端计划在等待发送变量里
...
function BJCBI_to_STMap(){
	...
	//json.l=l
	//发送给站场图
	addCookie('北京联锁',json.routeAllNum)
	//alert(json.routeAllNum)
	//alert(JSON.stringify($.cookie('北京联锁')))
	//清空存储列调计划的变量
